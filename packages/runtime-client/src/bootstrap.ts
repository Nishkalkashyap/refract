...
export async function bootstrapToolRuntime(
  ...
  for (const settled of settledModules) {
    if (settled.status === "rejected") {
      console.warn(
        "[refract] Failed to load runtime action module.",
        settled.reason
      );
      continue;
    }
    ...
    const runtimeAction = resolveRuntimeAction(action, moduleNamespace);
    if (runtimeAction) {
      runtimeActions.push(runtimeAction);
    }
  }

  const defaultActionId = resolveDefaultActionId(payload.defaultActionId, runtimeActions);
...
function resolveRuntimeAction(
  ...
  if (typeof moduleNamespace !== "object" || moduleNamespace === null) {
    console.warn(
      `[refract] Invalid runtime action module for '${action.id}'.`
    );
    return null;
  }
  ...
  if (!isToolRuntimeAction(runtimeExport)) {
    console.warn(
...
function isToolRuntimeAction(candidate: unknown): candidate is ToolRuntimeAction {
...
function resolveDefaultActionId(
